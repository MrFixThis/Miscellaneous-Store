...
<c:set var="manageOp" value="${action.equals('put') ? bookLot.getIsbn() : inventoryId}"/>
<!DOCTYPE html>
<html lang="en">
    <head>
    ...
    <body class="pt-5">
		...
		<div class="continer mt-4">
			<form action="/book_lots/manage/${action.equals('post') ? 'create' : 'update'}/${manageOp}">
				<h3 class="h3 text-muted text-center">
					<strong>Book Lot Information</strong>
				</h3>
						...
						<div class="input-group mb-3">
							<div class="input-group mb-3">
							  ...
							  <input type="text" name="isbn" class="TXT form-control"
								  value="${bookLot.getIsbn()}" readonly>
						  </div>
						</div>
					</div>
					<div class="text-success"><hr class="w-50 mx-auto"></div>
					...
					<div class="input-group mb-3">
						<div class="input-group mb-3">
							...
							<input name="name" type="text" class="TXT form-control"
								value="${bookLot.getName()}" maxlength="255" ${locker}>
						</div>
						<div class="input-group mb-3">
							<span class="input-group-text text-muted">Author(s) name(s)</span>
							<input name="authorName" type="text" class="TXT form-control"
								value="${bookLot.getAuthorName()}" maxlength="255" ${locker}>
						</div>
						<div class="input-group mb-3">
							<span class="input-group-text text-muted">Publisher name</span>
							<input name="publisherName" type="text" class="TXT form-control w-25"
								value="${bookLot.getPublisherName()}" ${locker}>
							<span class="input-group-text text-muted">Pages number</span>
							<input name="pagesNumber" type="text" class="NBR form-control"
								value="${bookLot.getPagesNumber()}" maxlength="255" ${locker}>
							...
							<input class="TXT form-control" type="date" name="publicationDate" placeholder="yyyy-mm-dd"
							 value="${bookLot.getPublicationDate().toString()}" min="1900-01-01" max="2030-12-31" ${locker}>
						</div>
						<div class="input-group mb-3">
					...
					<c:choose>
						...
						<c:otherwise>
							...
							<div class="btn-group me-4" role="group">
								<form action="/book_lots/update/${bookLot.getIsbn()}">
									<input type="submit" value="Edit Subject"
										class="btn btn-outline-info btn-lg mb-4">
								</form>
							</div>
							<div class="btn-group" role="group">
								<form action="/book_lots/manage/delete/${bookLot.getIsbn()}">
